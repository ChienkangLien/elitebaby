...
<%
NewsPhotoVO newsPhotoVO = (NewsPhotoVO) request.getAttribute("newsPhotoVO");
%>
...
<body class="c2">
	...
	<div id="main_div">
		...
		<FORM METHOD="post"
			ACTION="<%=request.getContextPath()%>/NewsPhoto.do" enctype="multipart/form-data"name="form1">
			<table>


				<jsp:useBean id="newsSvc" scope="page"
					class="com.tibame.web.service.LatestNewsService" />
				<tr>
					<td>消息編號:
<!-- 					<font color=red><b>*</b></font></td> -->
					<td><select size="1" name="newsId">
							<c:forEach var="latestnewsVO" items="${newsSvc.all}">
								<option value="${latestnewsVO.newsId}"
									${(param.newsId==latestnewsVO.newsId)? 'selected':'' }>${latestnewsVO.sortId}
							</c:forEach>
					</select></td>
				</tr>
				...
				<tr>
					<td>消息照片:
					<td><input type="file" class="form-control"name="newsPhoto" size="45" value="<%= (newsPhotoVO==null)? "" : newsPhotoVO.getNewsPhoto()%>" /></td>

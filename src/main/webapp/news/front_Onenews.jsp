...
<%
LatestNewsVO latestNewsVO = (LatestNewsVO)request.getAttribute("Msgs");
pageContext.setAttribute("lisit1", latestNewsVO);
%>

<!-- 留言 -->
<%;
pageContext.setAttribute("list", request.getAttribute("Msgs1"));
%>
    ...
    <div class="flex_container">
        <div class="flex_items -left">
                <img class="figure-img img-fluid rounded"
                    src="<%=request.getContextPath()%>/LatestNewsServlet?action=get_photo&&newsId=<%=latestNewsVO.getNewsId()%>">
        </div>
        <div class="flex_items-right">
            <h1 class="title1"><%=latestNewsVO.getPostTitle()%></h1>
            <p class="para"><%=latestNewsVO.getNewsIntro()%></p>
                <div class="comment-section">
                    <!--   留言顯示區塊-->
                    <div class="comment-list">
                    <p class = "GN-1box4A" id=comment_switch_up>
                    <img src="http://i2.bahamut.com.tw/gnn/bit.gif" class="IMG-E25">
                    <a href="javascript:open_comment()"></a>
                    </p>
                        <c:forEach var="newsMessageVO" items="${list}">

                                <div class="comment-item">
                                    <!--         			userId -->
                                    1:<span>${newsMessageVO.messageContent}</span>
  								</div>
                        </c:forEach>
                  </div>
                </div>                
				<FORM METHOD="post" ACTION="<%=request.getContextPath()%>/NewsMessage.do">
                            <!--     留言輸入區塊 -->
                        <div class="BH-1box GN-1box1" id="comment">
                            <div class="comment-input">
                                <p class="GN-1box1B">
                                    <input type="hidden" name="action" value="insert">
                                     <input type="hidden" name="userId" value="1">
                                     <input type="hidden" name="newsId" value="<%=latestNewsVO.getNewsId()%>">
                                    <input onkeypress="enterKey" name="messageContent" id="comment_input" type="textarea"
                                    placeholder="發表留言...(限30字)">
                                      <button type="submit" class="btn" id="comment_btn">發表</button>
                                </p>
                            </div>
                        </div>   
            	</FORM>   

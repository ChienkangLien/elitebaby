...
<%
LatestNewsVO lisit1 = (LatestNewsVO)request.getAttribute("Msgs");
pageContext.setAttribute("lisit1", lisit1);
%>
<!-- 留言 -->
<%
...
<section class="sec1 -type1">
    <div class="flex_container">
     <div class="flex_items -left">    
    		 <img class="figure-img img-fluid rounded" src="<%=request.getContextPath()%>/LatestNewsServlet?action=get_photo&&newsId=${lisit1.newsId}">
     </div>
     <div class="flex_items-right">
     <h1 class="title1" ><%=lisit1.getPostTitle()%></h1>
     <p class="para"><%=lisit1.getNewsIntro()%></p>
 
	<FORM METHOD="post" ACTION="<%=request.getContextPath()%>/NewsMessage.do">
  <input type="hidden" name="newsId" value="1"> <!--這裡的 value 可以依照頁面更改-->
  <div class="comment-section">
 <!--   留言顯示區塊-->
    <div class="comment-list">
      <c:forEach var="newsMessageVO" items="${list}">
        <c:if test="${newsMessageVO.newsId == 1}"> 
...
<!--     留言輸入區塊 -->
    ...
    <div class="comment-input">
      ...
      <input onkeypress="enterKey" name="messageContent" id="comment_input" type="textarea" placeholder="發表留言...(限30字)">
      <button type="submit" onclick="comment_ck" id="comment_btn">發表</button>
   		</p>
    </div>
  </div>
  </div>
